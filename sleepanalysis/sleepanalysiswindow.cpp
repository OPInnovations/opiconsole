...
//    qDebug() << "Total" << epochTotal <<"W"<< epochW << "P" << epochP << "M2"<< epochM2;
    ...
    if(doTxtFile)
    {
        ...
        outFileName.append(QString("D%1_%2_hyp_sum.txt").arg(datetimestr).arg(pdnDes));
        outfileTp = new QFile(outFileName);
        if(outfileTp->exists())
        {
            msgBox.setWindowTitle("Output Text File exists");
            msgBox.setText(QString("Out file %1 exists in selected directory.").arg(QString("D%1_%2_hyp_summary.txt").arg(datetimestr).arg(pdnDes)));
        ...
        *outTstrp << QString("<Sleep> (not for medical use)\n");
        ...
        // epoch scores
        outFileName = outDirName;
        outFileName.append(QString("D%1_%2_hyp.txt").arg(datetimestr).arg(pdnDes));
        outfileTp = new QFile(outFileName);
        if(outfileTp->exists())
        {
            msgBox.setWindowTitle("Output File exists");
            msgBox.setText(QString("Out file %1 exists in selected directory.").arg(QString("D%1_%2_hyp_epochscores.txt").arg(datetimestr).arg(pdnDes)));
            msgBox.setInformativeText("Do you want to overwrite?");
            msgBox.setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
            if(msgBox.exec() == QMessageBox::Cancel)
            {
    ...
    // EDF file
    outFileName = outDirName;
    outFileName.append(QString("D%1_%2_hyp.edf").arg(datetimestr).arg(pdnDes));
