...
{
	OPIPKT_DC10_t packageDC10 = {};

	// package payload structure can be found in the OPIWiredFrameDefinition_v1.10_release_20131122.pdf file

	packageDC10.dataCode = package.dataCode;
	packageDC10.length = package.length;
	memcpy(packageDC10.payload, package.payload, sizeof(package.payload));
	packageDC10.subDataCode = 0;

	// we can only handle the packets with data code 0x10
	if ((packageDC10.dataCode != 0x10) || (packageDC10.subDataCode != 0x00)) return packageDC10;
	...
	QString dateTime = qtTimeStamp.toString("yyyy-MM-dd HH:mm:ss.zzz");
	packageDC10.timeStampStr = dateTime.toLatin1().data();

	memcpy(&packageDC10.opiucd, &package.payload[11], 6);
	packageDC10.firmwareVersion = package.payload[17] << 8 + package.payload[18];
	packageDC10.mode = package.payload[19];
	
	memcpy(&packageDC10.associatedPDNs, &package.payload[20], 8);

	packageDC10.zigBeeChannel = package.payload[28];
	packageDC10.uSDType = package.payload[29];
	packageDC10.chargerStatus = package.payload[30];

	for (int i=0; i<8; i++)
	{
		memcpy(&packageDC10.pdnSettingsRaw[i], &package.payload[31+i*12], 12);

		if (packageDC10.associatedPDNs[i] != 0xFF)
		{
			packageDC10.pdnSettings[i].sensorDSN = ((long long)packageDC10.pdnSettingsRaw[i][0] << 32) + ((long long)packageDC10.pdnSettingsRaw[i][1] << 24) + ((long)packageDC10.pdnSettingsRaw[i][2] << 16) + ((long)packageDC10.pdnSettingsRaw[i][3] << 8) + (packageDC10.pdnSettingsRaw[i][4]);
			packageDC10.pdnSettings[i].firmwareVersion = packageDC10.pdnSettingsRaw[i][5] << 8 + packageDC10.pdnSettingsRaw[i][6];
			packageDC10.pdnSettings[i].zigBeeChannel = packageDC10.pdnSettingsRaw[i][7];
			packageDC10.pdnSettings[i].rfOutputPower = packageDC10.pdnSettingsRaw[i][8];
			packageDC10.pdnSettings[i].rfTransmitPower = packageDC10.pdnSettingsRaw[i][9];
			packageDC10.pdnSettings[i].rfTimeout = packageDC10.pdnSettingsRaw[i][10];
			packageDC10.pdnSettings[i].memoryModuleWrite = packageDC10.pdnSettingsRaw[i][11];
		}
	}
